 ...
 * @param {number} staggerDelay Delay between each element in milliseconds (default: 60).
 */
export function animateStaggerIn(targets, staggerDelay = 60) {
  anime({
    targets: targets,
    opacity: [0, 1],
    translateY: [15, 0],
...
/**
 * Animates an element fading in.
 * @param {HTMLElement} element The element to animate.
 * @param {number} duration Animation duration in milliseconds.
 */
export function animateFadeIn(element, duration = 400) {
  anime({
    targets: element,
    opacity: [0, 1],
    duration: duration,
