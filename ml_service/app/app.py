...
class TxCreate(BaseModel):
    ...
    amount: float
    currency: str
    transaction_type: Optional[str] = None  # "cash_in" or "cash_out"
        ...
        if cassandra_service.available():
            ...
            if isinstance(result, dict) and result.get("status") == "success":
                ...
                if isinstance(transaction_payload, dict):
                    _update_training_dataset_from_transaction(transaction_payload)
            return result
    except CassandraUnavailable:
        logger.warning("Cassandra unavailable, falling back to in-memory transaction store.")
    except Exception:
        logger.exception("Failed to write transaction to Cassandra; falling back to memory.")
    record = rec.dict()
    if isinstance(record.get("event_ts"), datetime):
        record["event_ts"] = record["event_ts"].isoformat()
    record["transaction_type"] = tx_type
    if tx_type:
        extra_payload = record.get("extra_json")
        if isinstance(extra_payload, dict):
            updated_extra = dict(extra_payload)
        else:
            updated_extra = {}
        updated_extra.setdefault("transaction_type", tx_type)
        updated_extra.setdefault("direction", tx_type)
        updated_extra.setdefault("client_tx_id", record.get("client_tx_id"))
        record["extra_json"] = updated_extra or None
    TX_STORE.append(record)
    _update_training_dataset_from_transaction(record)
    return {"status": "success", "transaction": record}

@app.post("/rt/transactions/bulk")
    ...
    for item in req.items:
        ...
        try:
            if cassandra_service.available():
                ...
                if isinstance(result_payload, dict) and result_payload.get("status") == "success":
                    tx_payload = result_payload.get("transaction")
                    if isinstance(tx_payload, dict):
                        _update_training_dataset_from_transaction(tx_payload)
            else:
                raise CassandraUnavailable
        except CassandraUnavailable:
            logger.warning("Cassandra unavailable during bulk insert; using memory store.")
        except Exception:
            logger.exception("Failed to write bulk transaction to Cassandra; using memory.")
        if result_payload is None:
            ...
            TX_STORE.append(record)
            _update_training_dataset_from_transaction(record)
            created.append({"status": "success", "transaction": record})
        else:
            created.append(result_payload)
    return {"status": "success", "count": len(created), "items": created}

@app.get("/rt/transactions")
...
@app.get("/rt/transactions/by-id/{tx_id}")
...
@app.get("/ml/m5p/feature-mapping")
def feature_mapping():
    model = getattr(m5p_model, "model", None)
    if not model:
...
@app.get("/ml/m5p/feature-importances")
